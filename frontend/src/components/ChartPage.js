...
const ChartPage = () => {
  const { objective, objectiveFunction, constraints, result } = useSelector(
  ...
  return (
    <div className="pt-5">
      ...
      <p>
        {objective}{' '}
        <span className="X">
          {objectiveFunction.a !== 1 ? objectiveFunction.a : ''}
          <span className="X plain-text">
            X<span className="number">{1}</span>
          </span>{' '}
          + {objectiveFunction.b !== 1 ? objectiveFunction.b : ''}
          <span className="X plain-text">
      ...
      <button
        type="button"
        onClick={() => dispatch(setDisplay('form'))}
      ...
      <Chart
        data={points}
        values={values}
        extended={points.length ? addMaxValues(constraints, points) : points}
      />
      <VariablesTable
        result={result}
      ...
      {result.status === 'optimal' ? (
          ...
          >
            <div className="card-body">
              <h5 className="card-title">
                The optimal solution is <b>Z = {result.solution.objective}</b>
              </h5>
              <p className="card-text">
                Obtained for values x1={result.solution.x1}, x2=
                {result.solution.x2}
              </p>
            </div>
          </div>
        ...
        <div className="d-flex justify-content-center align-items-center pt-2">
          ...
          >
            <div className="card-body">
              <h5 className="card-title">
                There is no solution to this problem, unbounded or infeasible
              </h5>
            </div>
          </div>
        </div>

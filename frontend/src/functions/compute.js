...
const getVariables = (constraint1,constraint2) => {
    const x1 = (constraint2.y*constraint1.b - constraint1.y*constraint2.b)/(constraint2.a*constraint1.b - constraint1.a*constraint2.b)
    const x2 = constraint1.b !== 0 ?  getX2(constraint1.a,constraint1.b,constraint1.y,x1) : getX2(constraint2.a,constraint2.b,constraint2.y,x1)
    return [x1,x2]
}
    ...
    constraints.forEach((item,i) => {
        ...
        sliced.forEach((elem,j) => {
            ...
            if(result[0] < 0 || result[1] < 0) {
                ...
                const temp = [...constraints].filter((_,k) => (k !== (j+i+1) && k !== i));
                temp.forEach( it => {
                    const isValid = compare(it.a,it.b,it.sign,it.y,result[0],result[1])
                    if(!isValid) {test = false}
                })
            }
            if(test) {
                const isThere = values.find((item) => (item.x1 === result[0] && item.x2 === result[1] ))
    ...
    constraints.forEach((item,i) => {
        if(item.b === 0){
            values.forEach((elem,j) => {
                ...
                }else if(points[i].coordinates.length < 2) {points[i].coordinates.push({x1: points[i].coordinates[0].x1,x2: points[i].coordinates[0].x2 === 0 ? 1 : 0});
            }               
            });
        }
        else if(item.a === 0){
                values.forEach((elem,j) => {
                    ...
                    }else if(points[i].coordinates.length < 2)  points[i].coordinates.push({x1: points[i].coordinates[0].x1 === 0 ? 1 : 0,x2: points[i].coordinates[0].x2});                     
                })   
        }
         else {
                values.forEach((elem,j) => { 
    ...
    if(!values){
        constraints.forEach((item,i) => {
            if(item.a === 0){
                ...
                points[i].coordinates.push({x1:  0 , x2: getX2(item.a,item.b,item.y,0)}) 
                console.log('x2: ',getX2(item.a,item.b,item.y,5) )
                points[i].coordinates.push({x1:  5 , x2: getX2(item.a,item.b,item.y,5)})                  
            }
        })
    }
...
export const addMaxValues = (constraints,points) => {
    ...
    constraints.forEach((item,i) => {
        if(item.a === 0){
            insertCoordinate(temp,i,{x1: maxValues[0],x2: temp[i]?.coordinates[0]?.x1})
        }
        else if(item.b === 0){
            insertCoordinate(temp,i,{x1:temp[i]?.coordinates[0]?.x1, x2: maxValues[1]})
        }
        else {
